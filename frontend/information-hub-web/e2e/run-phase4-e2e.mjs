...
const databaseUrl = firstEnvironment(
  'INFORMATION_HUB_E2E_DB_URL',
  'INFORMATION_HUB_TEST_DB_URL',
)
const databaseUsername = firstEnvironment(
  'INFORMATION_HUB_E2E_DB_USERNAME',
  'INFORMATION_HUB_TEST_DB_USERNAME',
)
const databasePassword = firstEnvironment(
  'INFORMATION_HUB_E2E_DB_PASSWORD',
  'INFORMATION_HUB_TEST_DB_PASSWORD',
)
const runKey = randomBytes(6).toString('hex')
const e2eUsername = `phase4-e2e-${runKey}`
const e2ePassword = `${randomBytes(24).toString('base64url')}Aa1!`
...
const rawPayloadMarker = `PHASE4_E2E_PRIVATE_RAW_MARKER_${runKey}`

validateTestDatabase(databaseUrl)
if (!existsSync(backendJar)) {
  throw new Error(
    `Backend JAR not found at ${backendJar}; run Maven package before Phase 4 E2E`,
  )
}

await provisionE2eAccount()
const provider = await startFakeProvider(providerApiKey, rawPayloadMarker)
...
try {
  ...
  await verifyCollectorAuthentication(backendUrl, collectorToken)
  ...
  const playwright = spawn(
    process.execPath,
    [playwrightCli, 'test', 'phase4.spec.ts', '--workers=1'],
    {
      cwd: webDirectory,
      env: {
        ...process.env,
        ...
        INFORMATION_HUB_E2E_PROVIDER_STATS_URL:
          `http://127.0.0.1:${provider.port}/e2e/stats`,
        INFORMATION_HUB_E2E_FORBIDDEN_PROVIDER_KEY: providerApiKey,
        INFORMATION_HUB_E2E_RAW_PAYLOAD_MARKER: rawPayloadMarker,
      },
      stdio: 'inherit',
      windowsHide: true,
    },
...
function startBackend({ backendPort, providerBaseUrl }) {
  return spawn(
    javaExecutable,
    [
      '-jar',
    ...
    ],
    {
      ...
      env: {
        ...
        INFORMATION_HUB_BOOTSTRAP_DISPLAY_NAME: 'Phase 4 E2E',
        ...
        INFORMATION_HUB_AI_MODEL: 'phase4-e2e-model',
        ...
        INFORMATION_HUB_AI_SCHEDULE_DISPATCHER_ENABLED: 'false',
        INFORMATION_HUB_RECOMMENDATION_WORKER_ENABLED: 'true',
        INFORMATION_HUB_RECOMMENDATION_WORKER_INITIAL_DELAY: '200ms',
        INFORMATION_HUB_RECOMMENDATION_WORKER_FIXED_DELAY: '200ms',
        INFORMATION_HUB_RECOMMENDATION_WORKER_STALE_AFTER: '30s',
      },
...
async function provisionE2eAccount() {
  const wrapper = process.platform === 'win32' ? 'mvnw.cmd' : './mvnw'
  const fixture = spawn(
    wrapper,
    ['-Dtest=Phase4E2eAccountFixtureTest', 'test'],
    {
      cwd: backendDirectory,
      env: {
        ...process.env,
        INFORMATION_HUB_TEST_DB_URL: databaseUrl,
        INFORMATION_HUB_TEST_DB_USERNAME: databaseUsername,
        INFORMATION_HUB_TEST_DB_PASSWORD: databasePassword,
        INFORMATION_HUB_E2E_FIXTURE_USERNAME: e2eUsername,
        INFORMATION_HUB_E2E_FIXTURE_PASSWORD: e2ePassword,
      },
  ...
  const code = await childExitCode(fixture)
  if (code !== 0) {
    throw new Error(`Phase 4 E2E account fixture exited with code ${code}`)
  }
}
...
function captureBackendOutput(
  ...
  for (const stream of [child.stdout, child.stderr]) {
    ...
    stream.on('data', (chunk) => {
      ...
      securityState.rawPayloadLeaked ||= chunk.includes(privateMarker)
...
async function verifyCollectorAuthentication(baseUrl, token) {
...
async function startFakeProvider(expectedApiKey, privateMarker) {
  let requestCount = 0
  const server = createHttpServer(async (request, response) => {
    if (request.method === 'GET' && request.url === '/e2e/stats') {
      respondJson(response, 200, { requestCount })
      return
    }
    if (
      request.method !== 'POST' ||
    ...
    if (serializedMessages.includes(privateMarker)) {
    ...
    const content = failure
      ...
      : JSON.stringify({
          ...
          relevanceScore: 88,
          confidence: 0.91,
          summary: 'Synthetic Phase 4 E2E analysis.',
    ...
    respondJson(response, 200, {
      id: `phase4-e2e-request-${requestCount}`,
...
function validateTestDatabase(url) {
  ...
  try {
    ...
    throw new Error('Phase 4 E2E database URL must be a valid JDBC URL')
  }
  if (
    !databaseName ||
    ...
    throw new Error(
      'Phase 4 E2E database name must contain test/e2e and must not contain dev',
    )
  }
}

function firstEnvironment(...names) {
  for (const name of names) {
    const value = process.env[name]
    if (value?.trim()) return value
  }
  throw new Error(`${names.join(' or ')} is required for Phase 4 E2E`)
}
