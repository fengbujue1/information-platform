...
Information Hub
    ...
    ├── recommendation（Phase 4 Profile、Interaction 与 Candidate 已实施）
    └── notification（后续）
    │
    ▼
MySQL
...
Phase 4 已启动，当前任务为 TASK-038。V4 Generic Recommendation Core + JOB Extension 数据库结构、Recommendation Profile API、View / Feedback / JOB disposition API，以及 JOB Candidate Resolver 已实施；算法、Run/Feed 与 Web 尚未实施。
